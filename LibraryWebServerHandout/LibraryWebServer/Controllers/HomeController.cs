...
//Other (Running LINQ Query and printing results)
/*using (TeamXLibraryContext db = new TeamXLibraryContext())
{
  var query =
  from t in db.Titles
  select t.Title;
...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin(string name, int cardnum)
        {
            ...
            using (Team31LibraryContext db = new Team31LibraryContext())
            {
                var query = from t in db.Patrons where t.Name == name && t.CardNum == cardnum select t;
        ...
        public ActionResult AllTitles()
        {
            ...
            using (Team31LibraryContext db = new Team31LibraryContext())
            {
                var query = from t in db.Titles
                            join i in db.Inventory on t.Isbn equals i.Isbn into inventory_titles
                            from j1 in inventory_titles.DefaultIfEmpty()
                            join c in db.CheckedOut on j1.Serial equals c.Serial into invetory_checkedout
                            from j2 in invetory_checkedout.DefaultIfEmpty()
                            join p in db.Patrons on j2.CardNum equals p.CardNum into inverntory_patrons
                            from j3 in inverntory_patrons.DefaultIfEmpty()
                            select new { isbn = t.Isbn, title = t.Title, author = t.Author, serial = j1 == null ? null:(uint?)j1.Serial, name = j3 == null ? "" : j3.Name };
        ...
        public ActionResult ListMyBooks()
        {
            ...
            using (Team31LibraryContext db = new Team31LibraryContext())
            {
                var query = from cd in db.CheckedOut
                            where cd.CardNum == card
                            join i in db.Inventory on cd.Serial equals i.Serial into checkedout_inventory
                            from j1 in checkedout_inventory.DefaultIfEmpty()
                            join t in db.Titles on j1.Isbn equals t.Isbn into checkedout_books
                            from j2 in checkedout_books.DefaultIfEmpty()
                            select new { title = j2.Title, author = j2.Author, serial = j1.Serial};
                return Json(query.ToArray());
            }
        }
        ...
        public ActionResult ReturnBook(int serial)
        {
            ...
            {
                //CheckedOut c = new CheckedOut();
                var query = from c in db.CheckedOut where c.CardNum == card && c.Serial == (uint)serial select c;
                //c.CardNum = (uint)card;
                //c.Serial = (uint)serial;
